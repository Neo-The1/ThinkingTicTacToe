...
class cnNetwork():
    ...
    def __init__(self, inputShape,outputSize,**kwds):
        ...
        self._layer1 = keras.layers.Conv2D(32,kernel_size=(3,3),strides=(1,1),activation='relu',
                                           padding='same')
        self._layerPool = keras.layers.MaxPool2D(pool_size=(2,2),strides=2)
        self._layer2 = keras.layers.Conv2D(16,kernel_size=(3,3),strides=(1,1),activation='relu',
                                           padding='same')
...
if __name__ == "__main__":
    ...
    testNet = cnNetwork((3,3,3),10)
#    states[0,:] = board.decodeState(board.getState())
#    print(states)
#    print(testNet.predict(states))
    result = testNet.predict(board.decodeStateCNN(board.getState()))
